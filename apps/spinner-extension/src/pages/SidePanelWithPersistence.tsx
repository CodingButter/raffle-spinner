...
function SidePanelContent() {
  ...
  // Session state that needs persistence
  ...
  const { saveSession, initializeSession } = useSessionPersistence({
    onSessionRestore: (session) => {
      ...
      setSpinTarget(session.spinTarget);
      
      // Don't restore spinning state - always start fresh
      setIsSpinning(false);
      setCurrentWinner(session.currentWinner || null);
    },
    onSessionExpired: () => {
      // Clear session state when expired
      setSessionWinners([]);
      setTicketNumber('');
      setSpinTarget('');
    ...
    onError: (errorMessage) => {
      ...
      // Don't show error to user - session persistence should be invisible
    }
  });
  ...
  useEffect(() => {
    const sessionState = {
      ...
      isSpinning: false, // Never persist spinning state
  ...
  const handleSpinComplete = async (winner: Participant) => {
    ...
    // Add to session winners - using Winner interface for UI component compatibility
