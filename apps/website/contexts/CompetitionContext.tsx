...
export function CompetitionProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveCompetitions = useCallback(async (
  ...
  const updateCompetition = useCallback(async (id: string, updates: Partial<Competition>) => {
    const newCompetitions = competitions.map(c =>
      c.id === id ? { ...c, ...updates } : c
    );
    setCompetitions(newCompetitions);
  ...
  const deleteCompetition = useCallback(async (id: string) => {
    ...
    await saveCompetitions(newCompetitions, selectedCompetition?.id === id ? null : selectedCompetition?.id || null);
  }, [competitions, selectedCompetition, saveCompetitions]);
